...
,game_data as (
    select t1.uin,t1.map_id,t1.game_session_id
    ,t1.game_dur
    ,case
...
select t2.uin,t2.map_id,t1.ctype
,count(game_session_id)                         as game_cnt
,sum(game_dur)                                  as game_dur
,count(if(frndgame_dur>0,game_session_id,null)) as frndgame_cnt
,sum(frndgame_dur)                              as frndgame_dur
,(select dt from args) as dt
from map_data t1
inner join game_data t2 on t1.map_id=t2.map_id
group by 1,2,3
;